...
// URL base per le API - SOSTITUISCI CON IL TUO URL VERCEL
const API_BASE_URL = 'https://tuo-progetto-vercel.vercel.app';
...
// Funzione per creare il link di pagamento Stripe - VERSIONE CORRETTA
async function creaLinkPagamento() {
  ...
  try {
    const API_BASE_URL = 'https://tuo-progetto-vercel.vercel.app';
    console.log("Chiamata API ->", `${API_BASE_URL}/api/crea-pagamento-stripe`);
    
    const response = await fetch(`${API_BASE_URL}/api/crea-pagamento-stripe`, {
      method: 'POST',
      headers: { 
    ...
    // Salva i dati prima del redirect
